...
def on_payment_succeeded(
    ...
    *,
    ...
    payment later succeeds through a path Day 8+'s recovery system never
    ...
    Without this, that kind of intent stays open forever and Day 8 wastes
    a decision on a loss that isn't one.

    Deliberately does NOT close a record that already has a
    recovery_attempts row: once Day 8 has decided something and Day 9 may
    have sent it, whether that success counts as a recovery -- and with
    what confidence -- is attribution's job (app/recovery/attribution.py),
    not this blunt a check. Closing it here first would set status to
    RECOVERED with no recovery_outcomes row at all, silently skipping
    attribution entirely -- exactly the bug this guard exists to prevent
    (caught during the Day 12 rehearsal: recovered_at outcomes were being
    generated by the world simulator but attribution kept reporting zero
    STRONG or WEAK matches, because on_payment_succeeded had already
    closed the record out from under it moments earlier in the same
    worker drain).
